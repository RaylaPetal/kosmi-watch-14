// Ported from Sebane1/XivMediaPlayer (AGPL-3.0) — XivMediaPlayer/Compositing/GlowRenderer.
// Unmodified apart from the namespace, per design.md Appendix C ("port as one unit").
using System;
...
/// <summary>
/// Creates a small, vignette-faded copy of the video texture on the GPU. The shader downsamples
/// and applies a smooth rectangular falloff so edges fade to transparent — no hard edges when
/// drawn as a glow quad behind a placed screen.
/// </summary>
internal sealed unsafe class GlowRenderer : IDisposable
{
...
";
    ...
    /// <summary>The SRV pointer for use with ImGui.AddImageQuad.</summary>
    ...
    public bool Initialize(int glowSize = 64)
    {
        ...
        try
        {
            ...
            {
                Width = (uint)_glowSize,
                Height = (uint)_glowSize,
                MipLevels = 1,
                ArraySize = 1,
    ...
    /// <summary>Renders the video texture into the glow texture with vignette falloff (GPU downsample + edge fade in one pass).</summary>
