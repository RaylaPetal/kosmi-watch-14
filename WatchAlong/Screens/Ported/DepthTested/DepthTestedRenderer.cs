// Ported from Sebane1/XivMediaPlayer (AGPL-3.0) — XivMediaPlayer/Compositing/DepthTestedRenderer.
// The shader (HLSL) and its matching constant-buffer structs are kept byte-for-byte: this file's
// only real risk is a cbuffer layout mismatch between the C# struct and the HLSL cbuffer, and it
// cannot be shader-compiled in this environment (no Windows D3DCompiler here) to verify an edit
// didn't break that alignment — so per design.md Appendix C ("port as one unit, don't rewrite"),
// this is ported unedited except for: namespace, removing the `AudioVisualState` type (WatchAlong
// has no audio-reactive visuals; its three cbuffer fields are always zero here instead), and
// dropping GBuffer2/GBuffer3/PreUITexture/VignetteExtrapolatedTexture wiring — none of those are
// ever actually sampled by the current shader body (PreUITexture/Vignette are read only behind
// `HasPreUI`, which XivMediaPlayer's own SceneColorProbe.GetToneAdjustSourceSrvPtr() now hard-codes
// to always return zero — "no longer used by Dawntrail" — so that branch is already dead upstream;
// GBuffer2/GBuffer3 are declared as shader resources but never `.Sample()`d anywhere in the PS).
// Everything gated behind ShowScreensaver/HoverUV/HasTitleTexture/IsLoadingOverlay/IsProjectorMode
// is inert given WatchAlong's call site (Screens/Ported/WorldVideoRenderer.cs) never sets those —
// left in the shader unedited rather than risk breaking cbuffer alignment by cutting it out.
using System;
...
/// <summary>Renders a textured quad with per-pixel depth occlusion using a fullscreen shader pass.</summary>
internal sealed unsafe class DepthTestedRenderer : IDisposable
{
    ...
    private ID3D11SamplerState _videoSampler = null!;
    private ID3D11SamplerState _depthSampler = null!;

    // Buffers
    private ID3D11Buffer _constantBuffer = null!;
    private ID3D11Buffer _visualFxConstantBuffer = null!;
    private ID3D11Buffer _uiRectBuffer = null!;
    ...
    // Per-frame copies of the composite RT so multiple screens can each keep their own ImGui texture.
    ...
    private struct PSConstants
    {
        ...
        // Appended at the end (see the block above): the crop this shader applies to sampleUV
        // ("Dynamic Resolution scaling" section of PS()) only ever multiplied by UVBottomEdge/
        // UVRightEdge (MaxV/MaxU) — correct only when the content's used region starts at (0,0)
        // within the source texture. WatchAlong's captured frames can have a real top offset (a
        // page header above the player that isn't always suppressed), making MinU/MinV nonzero —
        // without these, the shader sampled starting from row/col 0 (inside that header) instead
        // of the actual content start, showing as a border only in this render path (quad mode's
        // RenderScreenSpace already interpolates MinU/MinV..MaxU/MaxV correctly).
        public float UVTopEdge;
...
float4 PS(VS_OUT input) : SV_TARGET {
  ...
  if (abs(denom) > 1e-6) {
      ...
      sampleUV = uv;
      // No aspect-ratio correction here (unlike upstream): despite its pillarbox/letterbox naming
      // it actually center-crops/zooms sampleUV around 0.5 rather than adding bars, and triggers on
      // any >0.5% mismatch between the TV mesh's own aspect and the video's — which showed up as an
      // unwanted zoom/crop on screens not pinned to exactly 16:9. Quad mode (RenderScreenSpace) has
      // never done this correction and stretches contentUv directly onto the mesh; matching that
      // behavior here keeps the two render paths visually consistent. UV cropping to the video's
      // used sub-region (contentUv's MaxU/MaxV, independent of aspect) still applies unconditionally.
      sampleUV.y = UVTopEdge + (UVBottomEdge - UVTopEdge) * sampleUV.y;
      sampleUV.x = UVLeftEdge + (UVRightEdge - UVLeftEdge) * sampleUV.x;
  }
  ...
  if (isInside) {
      ...
      // Treat the screen surface as this much closer to the camera than it truly is, so a screen
      // placed close against its own backing wall doesn't read as self-occluded: reversed-Z depth
      // precision is coarse enough at typical viewing distances that a screen sitting a few cm off
      // its wall can fall within the 0.0001 compare epsilon below, tipping the PCF neighborhood
      // (and therefore the alpha multiply after it) toward fully occluded even though nothing is
      // actually in front of it. Validated worldspace-yalm clearance from the earlier CPU-side
      // depth-occlusion prototype.
      float clearanceYalms = 0.35;
      viewZ = max(0.01, viewZ - clearanceYalms);
  ...
  // UI-compositing tail disabled: it only ever writes color.rgb (never color.a), replacing it with
  // BackBufferTexture's captured pixel whenever bbAlpha reads as opaque there — and a typical
  // swapchain backbuffer doesn't carry reliable per-pixel alpha for that purpose, so bbAlpha reads
  // ~1.0 almost everywhere, silently overwriting the video with whatever was already behind the
  // screen (visually identical to nothing rendered, not a transparency/blend bug at all — found by
  // bisecting the shader with early return-color checkpoints against solid debug colors).
  // This feature (redrawing game UI like the MJI/ActionDetail/ToDoList HUD back over an occluded
  // screen) is a XivMediaPlayer-specific nicety WatchAlong doesn't need; cut rather than debug
  // further into backbuffer-alpha semantics that may not hold the way the original engine intends.
  return color;
  /* dead code below, kept for reference (port as one unit), never reached
  bool insideUI = false;
...
";
    ...
    /// <summary>Reset the per-frame output snapshot pool before rendering multiple screens.</summary>
    public void BeginMultiOutputFrame() => _outputCopyIndex = 0;

    /// <summary>Copy the current composite render target so ImGui can retain a stable texture per screen draw. The shared OutputSRV is overwritten on every Render call.</summary>
    ...
    public bool Initialize()
    {
        ...
        try
        {
            ...
            {
                ByteWidth = (uint)visualFxBufferSize,
                Usage = ResourceUsage.Default,
                BindFlags = BindFlags.ConstantBuffer,
                CPUAccessFlags = CpuAccessFlags.None,
            });
    ...
    public bool Render(
      ...
      float videoAspectRatio = 0, bool useDifferenceFallback = false, float opacity = 1.0f, bool isProjectorMode = false, Vector3? screensaverColor = null, int screensaverStyle = 0, float uiBlendThreshold = 0.0f, float uvBottom = 1.0f, float uvRight = 1.0f, float uvTop = 0.0f, float uvLeft = 0.0f, bool enableTvGlow = true, float loadingPulse = 0f, bool isLoadingOverlay = false, bool surfaceOnly = false, bool lightingOnly = false, bool staticLightSource = false,
      int visualEffectMode = 0, float effectIntensity = 0.65f, float effectSpeed = 1.0f)
    {
        ...
        // This call runs raw D3D11 immediate-context draws interleaved with the game's own render
        // loop (Dalamud's OnDraw hook fires mid-frame, not after it) — not inside an isolated pass
        // like Dear ImGui's own renderer, which always fully re-specifies its pipeline state before
        // drawing. Restoring only the render target (as XivMediaPlayer's original does) leaves our
        // vertex/pixel shaders, input layout, topology, blend state and viewport bound on the game's
        // shared context afterwards; if anything drawn later this frame assumes different state
        // instead of re-setting it all itself, that shows up as broken/invisible rendering for the
        // rest of the frame — matching both the "depth-tested composite is blank" and the "even the
        // plain-quad fallback goes invisible" reports, since both happen only once this GPU work runs.
        var savedViewports = new Vortice.Mathematics.Viewport[16];
        var savedViewportCount = 16u;
        _context.RSGetViewports(ref savedViewportCount, savedViewports);
        var savedInputLayout = _context.IAGetInputLayout();
        var savedTopology = _context.IAGetPrimitiveTopology();
        var savedVS = _context.VSGetShader();
        var savedPS = _context.PSGetShader();
        var savedVSConstantBuffers = new ID3D11Buffer[1];
        _context.VSGetConstantBuffers(0, savedVSConstantBuffers);
        var savedPSConstantBuffers = new ID3D11Buffer[3];
        _context.PSGetConstantBuffers(0, savedPSConstantBuffers);
        var savedPSSamplers = new ID3D11SamplerState[2];
        _context.PSGetSamplers(0, savedPSSamplers);
        var savedBlendState = _context.OMGetBlendState(out var savedBlendFactor, out var savedSampleMask);
        ...
        try
        {
            ...
            var constants = new PSConstants
            {
                ...
                // PreUITexture is always unbound here — XivMediaPlayer's own SceneColorProbe
                // hard-codes this pointer to zero too ("no longer used by Dawntrail").
                HasPreUI = 0.0f,
                ...
                HasIdleBranding = 0.0f,
                IdleBrandingAspect = 1.0f,
            ...
            // WatchAlong has no audio-reactive visuals (design.md non-goal) — always zero.
            var fxConstants = new VisualFxConstants
            {
                VisualEffectMode = visualEffectMode,
                EffectIntensity = effectIntensity,
                EffectSpeed = effectSpeed,
                AudioLevels = Vector4.Zero,
                AudioBandsLow = Vector4.Zero,
                AudioBandsHigh = Vector4.Zero,
            };
            ...
            // t0=Video t1=Depth t2=BackBuffer t3=Title t4=PreUI t5=GBuffer2 t6=GBuffer3
            // t7=Vignette t8=IdleBranding — only t0/t1/t2 are ever wired by WatchAlong; the rest
            // are provably dead given the constants above (see the file header comment).
        ...
        finally
        {
            ...
            for (uint i = 0; i < 9; i++)
                _context.PSSetShaderResource(i, (ID3D11ShaderResourceView)null!);

            if (savedViewportCount > 0)
                _context.RSSetViewports(savedViewportCount, savedViewports);
            _context.IASetInputLayout(savedInputLayout);
            _context.IASetPrimitiveTopology(savedTopology);
            _context.VSSetShader(savedVS);
            _context.PSSetShader(savedPS);
            _context.VSSetConstantBuffers(0, savedVSConstantBuffers);
            _context.PSSetConstantBuffers(0, savedPSConstantBuffers);
            _context.PSSetSamplers(0, savedPSSamplers);
            _context.OMSetBlendState(savedBlendState, savedBlendFactor, savedSampleMask);

            savedRTVs[0]?.Dispose();
            savedDSV?.Dispose();
            ownedSrvs?[0]?.Dispose();
            savedInputLayout?.Dispose();
            savedVS?.Dispose();
            savedPS?.Dispose();
            savedBlendState?.Dispose();
            foreach (var cb in savedVSConstantBuffers) cb?.Dispose();
            foreach (var cb in savedPSConstantBuffers) cb?.Dispose();
            foreach (var s in savedPSSamplers) s?.Dispose();
        }
    }
